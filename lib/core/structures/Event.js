...
class Event {
    constructor(obj) {
        ...
        if (typeof obj.once === 'boolean')
            this.once = obj.once;
        if (obj.forDm === 'only' || typeof obj.forDm === 'boolean')
            this.forDm = obj.forDm;
        if (typeof obj.forGuild === 'boolean')
            this.forGuild = obj.forGuild;
        this.run = obj.run;
    }
    static build(eventObj) {
        __1.bot.emit('eventBuild', new Event(eventObj));
    }
}
__decorate([
